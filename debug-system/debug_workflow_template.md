...
5. **📋 更新INDEX.md**: 记录该轮调试的时间、问题、进展、结论，并更新问题状态为"已解决"
6. **📝 生成总结报告**: 完成调试轮次的总结文档

**问题未彻底解决时的处理步骤**：

1. **🗂️ 执行本轮归档**: 将src中的文件按重要性分类到core/archive/deprecated目录
2. **📋 处理Bug列表**: 按上述步骤2的方式处理已解决和新发现的Bug
3. **📋 更新INDEX.md**: 记录该轮调试的时间、问题、进展、结论，并更新问题状态为"部分解决"或"调试中"
4. **📝 更新任务专用文档**: 在`debug_workflow_任务名.md`文档结尾处添加本轮进展描述以供下一轮调试复用，包括：
   - 本轮解决的问题
   - 当前遗留的问题
   - 下一步调试计划
   - 重要发现和经验
5. **🔄 准备下轮调试**: 创建下一轮调试环境，继续调试循环
